...
/*   Updated: 2022/04/06 15:54:06 by aachbaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	data->tex[0].addr = (int *)mlx_get_data_addr(data->tex[0].p_img,
			&data->tex[0].bpp, &data->tex[0].line_len,
			&data->tex[0].endian);
	data->tex[1].addr = (int *)mlx_get_data_addr(data->tex[1].p_img,
			&data->tex[1].bpp, &data->tex[1].line_len,
			&data->tex[1].endian);
	data->tex[2].addr = (int *)mlx_get_data_addr(data->tex[2].p_img,
			&data->tex[2].bpp, &data->tex[2].line_len,
			&data->tex[2].endian);
	data->tex[3].addr = (int *)mlx_get_data_addr(data->tex[3].p_img,
...
void	draw_texture(t_data *data, int x, int y)
{
	...
	while (y++ <= data->ray.drawend)
	{
		...
		if (y < data->info.win_h && x < data->info.win_w)
			data->img.addr[y * data->img.line_len
				/ 4 + x] = data->tex[data->t.tex_dir].addr[data->t.y_tex
				* data->tex[data->t.tex_dir].line_len / 4 + data->t.x_tex];
	}
}
